...
export async function POST(request: Request) {
    try {
        ...
        const { employeeId, fullName, email, department, wishes, selectedImages } = body;
        ...
        // Check employeeId length (must be 4 digits)
        if (!/^\d{4}$/.test(employeeId)) {
            return NextResponse.json(
                { error: "Mã nhân viên phải là 4 chữ số." },
                { status: 400 }
            );
        }
        ...
        // Generate unique random lucky number 000-999
        const allExistingNumbers = await prisma.identity.findMany({
            select: { luckyNumber: true },
        });

        const usedSet = new Set(allExistingNumbers.map((n: { luckyNumber: string }) => n.luckyNumber));
        if (usedSet.size >= 1000) {
            return NextResponse.json(
        ...
        while (true) {
            ...
            const strVal = rand.toString().padStart(3, "0");
